...
type ActivityFilter = 'all' | 'inProgress' | 'overdue' | 'today' | 'onHold' | 'upcoming' | 'completed';
...
export function ActivityStatusDashboard() {
  const { activities, updateActivityStatus, formatDateTime, simulationDate } = useContext(DataContext);
  const [filter, setFilter] = useState<ActivityFilter>('all');
  const { toast } = useToast();

  const filteredActivities = useMemo(() => {
    let sortedActivities = [...activities].sort((a, b) => new Date(a.deadline).getTime() - new Date(b.deadline).getTime());
    ...
    if (filter === 'completed') {
        return activities.filter(activity => activity.status === 'Completed').sort((a, b) => (b.endTime?.getTime() || 0) - (a.endTime?.getTime() || 0));
    }
    
    if (filter === 'inProgress') {
        return activities.filter(activity => activity.status === 'In Progress');
    }
    ...
    return sortedActivities.filter(activity => {
        ...
        switch (filter) {
            ...
            case 'overdue':
                return deadlineStart < simDateStart && activity.status !== 'Completed';
            case 'today':
                return deadlineStart.getTime() === simDateStart.getTime() && activity.status !== 'Completed';
            case 'upcoming':
                return deadlineStart > simDateStart && activity.status === 'Not Started';
            default:
                return true;
        }
    });
  ...
  return (
    <Card>
      <CardHeader>
        <CardDescription>
          Filter and manage all activities.
        </CardDescription>
         <div className="flex items-center gap-2 pt-2 flex-wrap">
            <Button variant={filter === 'all' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('all')}>All</Button>
            <Button variant={filter === 'inProgress' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('inProgress')}>In Progress</Button>
            <Button variant={filter === 'overdue' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('overdue')}>Overdue</Button>
            <Button variant={filter === 'today' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('today')}>Today's</Button>
        ...
        <TooltipProvider>
            <div className="max-h-[65vh] overflow-y-auto">
                ...
                <TableHeader>
                    ...
                    {filter === 'inProgress' && <TableHead>Source</TableHead>}
                    <TableHead className="text-right">Actions</TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody>
                    {filteredActivities.length > 0 ? filteredActivities.map((activity) => {
                        const source = getSource(activity, simulationDate);
                        return (
                                ...
                                {filter === 'inProgress' && (
                                    <TableCell>
                                        <Badge 
                                            variant="outline"
                                            className={cn(
                                                source === 'Overdue' && 'border-destructive text-destructive',
                                                source === 'Today' && 'border-blue-500 text-blue-500',
                                                source === 'On Hold' && 'border-yellow-500 text-yellow-500',
                                                source === 'In Progress' && 'border-primary text-primary',
                                                source === 'Completed' && 'border-green-600 text-green-600'
                                            )}
                                        >
                                            {source}
                                        </Badge>
                        ...
                        <TableRow>
                            <TableCell colSpan={filter === 'inProgress' ? 10 : 9} className="h-24 text-center text-muted-foreground">
