...
export function DataProvider({ children }: { children: ReactNode }) {
    ...
    const saveData = useCallback(
        (newPackages: Package[], newActivities: Activity[]) => {
    ...
    const updateActivityStatus = useCallback(
        (activityId: string, newStatus: ActivityStatus, reason?: string, remarks?: string) => {
            let toastMessage:
            ...
            setActivities(prev => {
                ...
                if (newStatus === 'Completed' && oldStatus === 'Not Started') {
                    toast({
                        ...
                        description:
                            'Please start the activity before marking it as complete.',
                        variant: 'destructive',
                    });
                    return prev;
                }
    ...
    const updatePackageDates = (
        ...
        return currentPackages.map(p =>
            p.id === packageId
                ? { ...p, startDate: minStartDate, endDate: maxEndDate }
                : p
        );
    };

    const addOrUpdatePackage = useCallback(
        async (pkg: Package): Promise<void> => {
            setPackages(prev => {
        ...
        async (activity: Activity): Promise<void> => {
            setActivities(prevActivities => {
                ...
                const tagActivities = tempActivities
                    .filter(
                        a => a.packageName === activity.packageName && a.tag === activity.tag
                    )
                    .sort((a, b) => a.deadline.getTime() - b.deadline.getTime());

                const currentActivityIndexInTag = tagActivities.findIndex(
                    a => a.id === activity.id
                );

                if (currentActivityIndexInTag > -1) {
                    for (
                        let i = currentActivityIndexInTag + 1;
                        i < tagActivities.length;
                        i++
                    ) {
                        ...
                        const duration = differenceInMilliseconds(
                            currentActivityInTag.plannedEndDate,
                            currentActivityInTag.deadline
                        );
                        const newDeadline = new Date(prevActivityInTag.plannedEndDate.getTime());
                        const newPlannedEndDate = new Date(
                            newDeadline.getTime() + duration
                        );

                        const globalIndexOfCurrent = tempActivities.findIndex(
                            a => a.id === currentActivityInTag.id
                        );
                ...
                setPackages(currentPackages => {
                    const updatedPackages = updatePackageDates(
    ...
    const bulkAddData = useCallback(
        async (data: { packages: Package[]; activities: Activity[] }): Promise<void> => {
    ...
    const updateTagForActivities = useCallback(
        async (activityIds: string[], newTag: string): Promise<void> => {
            setActivities(prevActivities => {
